...
\caption{RBS data for the \Ge{76} target near the beamspot.  The plateau at low energies is due to small holes in the target; these allow the incident $\alpha$ particles to travel through to the aluminum chamber.  The plateau at high energies is due to small sections of the target with no \Ge{76} deposited on the gold surface.  The fit to these ``holes'' in the target is shown in (a).  The adjusted RBS spectrum and its fit are shown in (b).  Fitting the RBS spectrum with and without adjusting for holes does not result in any significant differences to the predicted target composition or thickness.}
...
\centering
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/atLeastOneCorrelatedBar.eps}
   \caption{Energy spectrum of a neutron detector bar.  Entries in this histogram are from events having at least one correlated signal in the neutron detector and are likely to be muons.  Notice the high-energy-deposition cosmic peak at $\sim$ channel~3000 and its considerable height relative to the low-energy $\gamma$ background.  This low-energy background is dominant in non-muon events as in {\fig}~\ref{fig:notMuonVetoSpectrum}.}
   \label{fig:muonVetoSpectrum}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/singleBar_noVeto.eps}
   \caption{Neutron detector bar energy spectrum.  These events have no correlation to other signals in the neutron detector and are unlikely to be due to muons.  While some high-energy depositions are visible, the spectrum is clearly dominated by the low-energy peak due to $\gamma$-radiation.}
   \label{fig:notMuonVetoSpectrum}
}
\subfloat[][]{
   \includegraphics[width=1.0\textwidth]{figures/vetoGroups.eps}
   \caption{Groups of detectors chosen to veto bars in the neutron detector.  If the events in a neutron detector bar vetoed by a detector have an energy spectrum dominated by the low-energy $\gamma$-radiation, that detector is not used as part of the veto for that particular bar.}
...
\label{fig:productPositionDependence}
\end{figure}
...
\subfloat[][]{
   ...
   \caption{The position dependence of the light signal from a single PMT.}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/position_sliceGraphs.eps}
   \caption{The two-dimensional histogram shown in (a) is projected into a series of one-dimensional histograms.  The peaks of these histograms are determined by fitting with a Landau function.}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/EnergyCutExample.eps}
   \caption{A plot describing the curve which, when scaled, is used as the low-energy cut.  The points are determined from the histograms shown in (b).  Points at the extremes that do not follow the trend are used to determine the position cut.}
}
\caption{}
...
\centering
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/scalingFactor.eps}
   \caption{Applying a scaling factor to the cut determined as in {\fig}~\ref{fig:fits_pkVSpos} is measured relative to the background obtained with the unscaled cut.  The effect of the highlighted scaling factors is shown in {\fig}~ref{fig:highEnergyCut}.}
   \label{fig:scalingFactor}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/needLowerCut.eps}
   \caption{Without applying a scaling factor to the cut determined as in {\fig}~\ref{fig:fits_pkVSpos}, the neutron signals are nearly lost.  Scaling the cut by $\sim$0.5 results in a reasonable signal to noise ratio.}
...
\centering
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/74Ge_limits.eps}
   \caption{}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/74Ge_2sigma_limit.eps}
   \caption{}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/76Ge_limits.eps}
   \caption{}
}
\subfloat[][]{
   \includegraphics[width=0.5\textwidth]{figures/76Ge_2sigma_limit.eps}
   \caption{}
}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/nickelTrend.eps}
...
& $V_0$ (MeV) & $r_R$ (fm) & $r_C$ (fm) & $a_R$ (fm) & W (MeV) & $r_l$ (fm) & $a_l$ (fm) & $W_D$ (MeV) & Norm. Factor\\
Becchetti & & & 1.25 & 1.137 \\
GDP08 & 118.3 & 1.30? & 1.24 & 0.82? & 34.2? & 1.31? &0.84? & 106.4? & 0.904 \\
Urone & 175.4 & 1.14 & 1.4 & 0.71 & 19.9 & 1.53 & 0.85 & 9.13(N-Z)/A & 0.910 \\
\end{tabular}
